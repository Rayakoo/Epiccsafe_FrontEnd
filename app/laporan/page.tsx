...
function PersonalPage() {
  ...
  useEffect(() => {
    ...
    if (urlParam) {
      setForm(prev => ({ ...prev, url: urlParam }))
    }
  }, [searchParams])
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        risk_score: String(res.risk_score),
        url: form.url,
      })
      router.push(`/succes?${params}`)
    } catch (err) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-6 md:gap-7 items-start">
          ...
          <div className="bg-[#2C2C2C] border-none rounded-2xl overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.4)] animate-fadeUp [animation-delay:0.15s] order-1 lg:order-2">
            ...
            <form onSubmit={handleSubmit} className="p-5 md:p-7 flex flex-col gap-4 md:gap-4.5">
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <input
                      ...
                      className="w-full bg-[#4A4A4A] border border-white/10 rounded-lg text-[#ECECEC] font-sans text-[13.5px] p-3 outline-none focus:border-[#E8001D] focus:shadow-[0_0_0_3px_rgba(232,0,29,0.15)] transition-all"
                    />
                    <input
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="flex flex-col gap-2">
                    <label className="text-[12px] md:text-[13px] font-semibold text-[#ECECEC]">Provinsi</label>
                    <select
                      value={form.province}
                      onChange={e => updateField('province', e.target.value)}
                      className="w-full bg-[#4A4A4A] border border-white/10 rounded-lg text-[#ECECEC] font-sans text-[13.5px] p-3 outline-none focus:border-[#E8001D] focus:shadow-[0_0_0_3px_rgba(232,0,29,0.15)] transition-all appearance-none"
                    >
                      <option value="" className="bg-[#4A4A4A]">Pilih Provinsi</option>
                      {PROVINSI_LIST.map(p => (
                        <option key={p} value={p} className="bg-[#4A4A4A]">{p}</option>
                      ))}
                    </select>
                  </div>
                  <div className="flex flex-col gap-2">
                    <label className="text-[12px] md:text-[13px] font-semibold text-[#ECECEC]">Usia</label>
                    <input
                      type="text" inputMode="numeric" placeholder="Contoh: 25"
                      value={form.age}
                      onChange={e => updateField('age', e.target.value)}
                      onKeyDown={e => {
                        if (!/^\d$/.test(e.key) && e.key !== 'Backspace' && e.key !== 'Delete' && e.key !== 'Tab' && e.key !== 'ArrowLeft' && e.key !== 'ArrowRight' && e.key !== 'Home' && e.key !== 'End') {
                          e.preventDefault()
                        }
                      }}
                      onPaste={e => {
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <input
                    type="url" placeholder="https://tautan-mencurigakan.xyz/.."
                    value={form.url}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                    {['SMS', 'WhatsApp', 'Email'].map(s => (
                      <button
                        key={s}
                        type="button"
                        onClick={() => updateField('source', s)}
                        className={`flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg border-2 font-sans text-[14px] font-semibold cursor-pointer transition-all ${
                          form.source === s
                            ? 'border-[#E8001D] bg-[rgba(232,0,29,0.12)] text-white'
                            : 'border-white/10 bg-[#4A4A4A] text-[#ECECEC] hover:border-[rgba(232,0,29,0.5)]'
                        }`}
                      >
                        <span className={`w-3 h-3 rounded-full border-2 ${
                          form.source === s ? 'bg-[#E8001D] border-[#E8001D]' : 'bg-white/25 border-white/30'
                        }`}></span>
                        {s}
                      </button>
                    ))}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <textarea
                    placeholder="Ceritakan secara singkat apa yang terjadi, kapan, dan bagaimana kamu menemukan / mengalami kejadian ini...."
                    rows={5}
                    value={form.description}
                    onChange={e => updateField('description', e.target.value)}
                    className="w-full bg-[#4A4A4A] border border-white/10 rounded-lg text-[#ECECEC] font-sans text-[13.5px] p-3 outline-none resize-y leading-relaxed focus:border-[#E8001D] focus:shadow-[0_0_0_3px_rgba(232,0,29,0.15)] transition-all"
                  ></textarea>
                </div>
                ...
                {error && (
                  <div className="bg-[rgba(232,0,29,0.12)] border border-[rgba(232,0,29,0.3)] rounded-lg p-3 text-[13px] text-[#E8001D] font-semibold">
                    {error}
                  </div>
                )}
                ...
                <div className="flex justify-end mt-4 md:mt-6 pt-0 border-t-0">
                  <button
                    type="submit"
                    disabled={loading}
                    className="w-full sm:w-auto bg-[#E8001D] text-white border-none rounded-lg px-8 md:px-10 py-3 font-sans text-[15px] font-bold tracking-[0.3px] cursor-pointer hover:bg-[#FF001F] hover:shadow-[0_4px_20px_rgba(232,0,29,0.45)] hover:-translate-y-0.5 transition-all active:translate-y-0 disabled:opacity-60 disabled:cursor-not-allowed"
                  >
