...
export default function HasilClient() {
  ...
  return (
    <div className="min-h-screen bg-[#140606] text-white flex flex-col">
      ...
      <nav className="sticky top-0 z-100 bg-[#2a0606] border-b border-white/7 h-[60px]">
        <div className="max-w-[1280px] mx-auto h-full flex items-center justify-between px-6 gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <a href="/login" className="flex items-center gap-2 no-underline text-white text-[13px] font-semibold px-4 py-1.5 border-2 border-white/25 rounded-full hover:bg-white/10 hover:border-white/40 transition-all">
              Login
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-12 gap-8 animate-fadeUp">
        {/* Result Card */}
        <div className="bg-white/7 border border-white/10 rounded-2xl p-11 w-full max-w-[760px] flex flex-col items-center gap-2.5 animate-cardIn">
          <h1 className="text-[clamp(22px,3vw,30px)] font-extrabold text-white leading-tight -tracking-[0.02em] text-center">
            Hasil Analisis AI
          </h1>
          <p className="text-[14.5px] text-white/60 text-center mb-3">Berikut hasil deteksi otomatis dari sistem EPICCSAFE</p>

          {/* Score Section */}
          <div className="w-full flex flex-col items-center gap-1.5 mb-2">
            <span className="text-[11.5px] font-bold text-white/35 tracking-[0.14em] uppercase">Risk Score</span>
            <div className={`text-[clamp(56px,8vw,80px)] font-extrabold leading-none -tracking-[0.04em] ${config.textColor}`}>
              {score}
            </div>
          ...
          {/* Status Banner */}
          <div className={`w-full ${config.bgBanner} ${config.borderBanner} rounded-xl p-5 text-center flex flex-col gap-1.5 mt-2`}>
            <span className={`text-[15px] font-extrabold tracking-[0.08em] uppercase ${config.statusColor}`}>{config.statusText}</span>
        ...
        {/* Action Buttons */}
        <div className="flex items-center justify-end gap-3.5 w-full max-w-[760px] animate-cardIn [animation-delay:0.15s]">
          <a href="/deteksi" className="no-underline text-white text-[16px] font-bold px-9 py-3.5 border-2 border-white/30 rounded-lg bg-transparent hover:bg-white/8 hover:border-white/50 transition-all">
            Kembali
          </a>
          <a href="/laporan" className="no-underline text-white text-[16px] font-bold px-9 py-3.5 rounded-lg bg-[#e03030] border-2 border-transparent hover:bg-[#ff4444] hover:-translate-y-0.5 transition-all">
            Laporkan
          </a>
        </div>
      </main>
