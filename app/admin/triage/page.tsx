...
export default function TriagePage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const data = await getReports('IN_REVIEW')
        const active = data.filter(r => r.status === 'IN_REVIEW')
        active.sort((a, b) => (b.risk_score ?? 0) - (a.risk_score ?? 0))
        setReports(active)
      } catch (err: unknown) {
  ...
  const filtered = reports.filter(r => {
    ...
    if (riskFilter === 'HIGH') return (r.risk_score ?? 0) >= 70
    if (riskFilter === 'MEDIUM') return (r.risk_score ?? 0) >= 40 && (r.risk_score ?? 0) < 70
    if (riskFilter === 'LOW') return (r.risk_score ?? 0) < 40
    return true
  })
    ...
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <main className="flex-1 p-4 md:p-8 flex flex-col gap-4 md:gap-5">
        ...
        <div>
          ...
          <p className="text-[11px] md:text-[12.5px] text-[#6B7E93] mt-0.5">{reports.length} tiket menunggu</p>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {TABS.map(tab => (
            <button
              ...
              className={`px-5 py-1.5 rounded-lg border text-[13px] font-semibold transition-colors cursor-pointer ${
                riskFilter === tab.key
        ...
        <div className="bg-[#1B2A3B] border border-white/10 rounded-xl md:rounded-2xl">
          ...
          <div className="overflow-x-auto">
            {loading ? (
              ...
              <div className="p-6 text-center text-[#6B7E93]">Tidak ada tiket menunggu</div>
            ) : (
